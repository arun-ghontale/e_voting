...
def dashboard(request):
    ...
    is_verified = True
    if not is_verified:
        return face_reco_render(request)
...
def face_reco(request):
    if request.method != 'POST':
        ...
        current_face_location = face_recognition.face_locations(img)
        current_face_encoding = face_recognition.face_encodings(img, known_face_locations=current_face_location)
        ...
        if len(current_face_encoding) > 0:
            ...
            # Change threshold if required
            if matches[match_ind] < 0.55:
                context = {
                    'is_verified': True,
