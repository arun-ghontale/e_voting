...
class AccountCheckMiddleWare(MiddlewareMixin):
    def process_view(self, request, view_func, view_args, view_kwargs):
        ...
        if user.is_authenticated:
            ...
            if any([request.path == reverse('account_login'),
                    request.path == reverse('account_register'),
                    modulename == 'django.contrib.auth.views',
                    request.path == reverse('account_login'),
                    request.path == reverse('home_page'),
                    request.path == reverse('links'),
                    request.path == reverse('ect'),
                    request.path == reverse('p2')]):
                pass
